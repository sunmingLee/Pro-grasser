...
public class Main_BOJ_1504_특정한최단경로_G4_윤성도_500ms {
	...
	public static void main(String[] args) throws Exception {
		...
		int ans = Math.min(d2[1] + d2[v2] + d3[N], d3[1] + d3[v1] + d2[N]);
		System.out.print(ans < INF ? ans : -1);
	}
	static void dijkstra(Node[] adj, int[] d, int start, int des, int N) {
		PriorityQueue<Node> q = new PriorityQueue<>();
		Arrays.fill(d, INF);
		q.offer(new Node(start, 0));
		while (!q.isEmpty()) {
			if (d[des] != INF && d[1] != INF && d[N] != INF)
				return;
			Node cur = q.poll();
			if (d[cur.to] != INF)
				continue;
