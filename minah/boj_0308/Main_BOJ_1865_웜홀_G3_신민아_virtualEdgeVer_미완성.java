...
public class Main_BOJ_1865_웜홀_G3_신민아_virtualEdgeVer_미완성 {
	static class Node{
		int end, weight;
	...
	public static void main(String[] args) throws Exception {
		...
		while(TC -- > 0) {
			...
			sb.append(bellmanFord(nodes) ? "YES\n" : "NO\n");
	...
	static private boolean bellmanFord(List<Node>[] nodes) {
		int[] distances = new int[nodes.length]; // 최단 거리
		Arrays.fill(distances, Integer.MAX_VALUE); // 초기화 by maxValue
		distances[0] = 0; // 시작점은 0으로부터 시작
		
		int edges = nodes.length - 1;
