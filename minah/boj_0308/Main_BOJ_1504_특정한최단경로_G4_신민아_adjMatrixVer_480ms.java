...
public class Main_BOJ_1504_특정한최단경로_G4_신민아_adjMatrixVer_480ms {
	...
	public static void main(String[] args) throws Exception {
		...
		int[][] nodes = new int[edges+1][edges+1];
		...
		for(int i=0;i<vertexes;i++) {
			...
			nodes[from][to] = weight; // 도로가 양방향이기 때문에 양쪽으로 길을 내준다
	...
	static private int getShortestPath(int start, int end, int[][] nodes) {
		...
		while(!pQueue.isEmpty()) { // 더 이상 갈 곳이 없을 때까지
			...
			// 계산 부분
			for(int i=1;i<nodes.length;i++) { // 연결 된 정점을 계속 돌기
				int tempDistance = distances[current.end] + nodes[current.end][i]; // 임시 거리 : 출발지~현재까지의 거리와 현재로부터 다음 정점까지의 거리 더하기
				
				if(!visited[i] && nodes[current.end][i] != 0 && distances[i] > tempDistance) { // 새로 갱신된 거리가 짧을 시
					distances[i] = tempDistance; // 갱신
					pQueue.offer(new Node(i, distances[i])); // queue에 이동할 정점이라고 넣어줌
				}
			}
		}
	...
	static private int getTotalPath(int[] points, int[][] nodes) {
		int totalDistance = 0;
