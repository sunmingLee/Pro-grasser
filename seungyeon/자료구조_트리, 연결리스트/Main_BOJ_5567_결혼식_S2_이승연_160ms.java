...
public class Main_BOJ_5567_결혼식_S2_이승연_160ms {
	public static void main(String[] args) throws Exception {
		...
		ArrayList<ArrayList<Integer>> list = new ArrayList<ArrayList<Integer>>(); // 인접 리스트 
		...
		for(int i=0; i<M; i++) {
			...
			list.get(a1).add(b1); // 양방향 (a1의 친구 b1)
			list.get(b1).add(a1); // 양방향 (b1의 친구 a1)
		}
		...
		for(int i=0; i<2; i++) { // depth 2번까지 
			...
			for(int j=0; j<size; j++) {
				...
				for(int k=0; k<list.get(e).size(); k++) { // 자기 자식의 노드들을 넣어줌 
					queue.offer(list.get(e).get(k));
					invite[list.get(e).get(k)] = true; // 초대할 친구 체크 
				}
			}
		}
